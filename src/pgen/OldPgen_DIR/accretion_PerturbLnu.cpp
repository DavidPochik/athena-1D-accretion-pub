...
// floor temperature.
static Real Coeff_nu_0, Coeff_nubar_0, t_L_0, t_L_1, t_coeff, dCoeff_nu, dCoeff_nubar, eps_nue, eps_nueb, L_nue, L_nueb, T_floor, dLnue, dLnueb;
// Machine epsilon.
...
Real YeTejas(Real temp, Real ye, Real x, Real Rho, Real Lnue_t, Real Lnueb_t) {
...
Real Heating_QW_Modified(Real x, Real Ye, Real Lnue_t, Real Lnueb_T) {
...
//                   (8.0 * PI * mn * std::pow(E_e,2.0) * 1.0e12); // Heating prefactor in erg s^-1 g^-1
 ...
 // Real H = prefactor * F * (Xn * L_nue * std::pow(kbT_nue,2.0) + Xp * L_nueb * std::pow(kbT_nueb,2.0)) * (1.0 - x) / std::pow(r_0/1.0e6,2.0);
  Real H = MeV2erg * prefactor * F * (Xn * Lnue_t * std::pow(kbT_nue,2.0) + Xp * Lnueb_t * std::pow(kbT_nueb,2.0)) * (1.0 - x) / std::pow(r_0,2.0); // erg/s/g
  return H;
}
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  dLnue        = pin->GetReal("problem","d_Lnue")
  dLnueb       = pin->GetReal("problem","d_Lnueb")
...
//        if (temp < 1.0) {
        ...
        Real f        = (time <= t_L_0) ? 0.0 :
                        ((time >= t_L_1) ? 1.0 : SQR(std::sin((time - t_L_0) * t_coeff)));
        Real L_nue_t  = L_nue  + f * dLnu;
        Real L_nueb_t = L_nueb + f * dLnue;
...
//        Real vdYedr = std::min(1.0, X_n) * (YeSource_QW_Modified(rho,temp,Ye_i,x,r) - YeSink_QW_Modified(rho,temp,Ye_i,x,r));
        Real vdYedr = ExpSupp * std::min(1.0,X_n) * YeTejas(temp, Ye_i, x, rho, L_nue_t, L_nueb_t);
//        Real qdot   = std::min(1.0, X_n) * (qdotScheck_H(Ye_i,r) - qdotScheck_C(temp/kbol_MeV,Ye_i,rho,r)); // erg/s/g units
     //   Real qdot = std::min(1.0,X_n) * (Heating_QW_Modified(x, Ye_i) - Cooling_QW_Modified(rho, temp, Ye_i));
      //  Real qdot = qdotQW(temp, x, Ye_i, time) / erg2MeV;
        Real qdot   = ExpSupp * std::min(1.0,X_n) * (Heating_QW_Modified(x, Ye_i, L_nue_t, L_nueb_t) - Cooling_QW_Modified(rho, temp, Ye_i)); // erg/s/g
//        std::cout << "qdot = " << qdot << " erg/s/g" << std::endl;
