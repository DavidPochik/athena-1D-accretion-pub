...
void InflowInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
  ...
  for (int k=ks; k<=ke; ++k) {
    ...
    //      Real Teq      = Arr[0];
          ...
          pmb->pscalars->r(ye_index,k,j,is-i) = Yeq;
...
void MeshBlock::UserWorkInLoop() {
  ...
  Real energy_term = (std::pow(eps_nue * fermi2 / fermi3, 2.0) * fermi4 / fermi2 + 2.0 * delta_np * (eps_nue * fermi2 / fermi3) * fermi3 / fermi2 
                    + std::pow(delta_np,2.0)) / std::pow(mcsq, 2.0);
