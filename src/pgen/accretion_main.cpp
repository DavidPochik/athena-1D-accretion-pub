...
static Real T_eq, Ye_eq, Mach, mu, rho_0, rho_f, v_f, p_f, r_0, Na, B_0, alpha, ye_0, ye_f;
static Real eps_nue, eps_nueb, L_nue, L_nueb;
// Machine epsilon.
...
Real YeTejas(Real temp, Real ye, Real x, Real Rho) {
  ...
  Real ferm5 = fermi(5,0); 
  Real ferm4 = fermi(4,0); 
  Real ferm3 = fermi(3,0); 
  Real ferm2 = fermi(2,0); 
...
void InflowInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
  ...
  // perturb base density every n'th cycle, avoiding half-timesteps
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=1; i<=ngh; ++i) {
          ...
          if (NON_BAROTROPIC_EOS) {
            prim(IPR,k,j,is-i) = pmb->peos->PresFromRhoT(rho_new, T_g, r_scalar);
          }
      }
    }
  }
